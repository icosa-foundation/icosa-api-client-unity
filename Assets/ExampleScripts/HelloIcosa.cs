...
/// </summary>
public class HelloIcosa : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        IcosaApi.GetAsset($"assets/{assetId}", GetAssetCallback);
        statusText.text = "Requesting...";
    }

    // Callback invoked when the featured assets results are returned.
    private void GetAssetCallback(IcosaStatusOr<IcosaAsset> result)
    {
        if (!result.Ok)
        {
        ...
        IcosaApi.Import(result.Value, options, ImportAssetCallback);
    }
