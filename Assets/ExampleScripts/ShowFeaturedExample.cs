...
public class ShowFeaturedExample : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        IcosaListAssetsRequest request = IcosaListAssetsRequest.Featured();
        // Limit requested models to those of medium complexity or lower.
        request.maxComplexity = IcosaMaxComplexityFilter.MEDIUM;
        IcosaApi.ListAssets(request, ListAssetsCallback);
    }

    // Callback invoked when the featured assets results are returned.
    private void ListAssetsCallback(IcosaStatusOr<IcosaListAssetsResult> result)
    {
        if (!result.Ok)
        {
        ...
        IcosaImportOptions options = IcosaImportOptions.Default();
        ...
        List<IcosaAsset> assetsInUse = new List<IcosaAsset>();
        for (int i = 0; i < Mathf.Min(10, result.Value.assets.Count); i++)
        {
            // Import this asset.
            IcosaApi.Import(result.Value.assets[i], options, ImportAssetCallback);
            assetsInUse.Add(result.Value.assets[i]);
        }

        // Show attributions for the assets we display.
        attributionsText.text = IcosaApi.GenerateAttributions(includeStatic: true, runtimeAssets: assetsInUse);
    }

    // Callback invoked when an asset has just been imported.
    private void ImportAssetCallback(IcosaAsset asset, IcosaStatusOr<IcosaImportResult> result)
    {
        if (!result.Ok)
        {
