...
namespace IcosaToolkitDev
{
    internal class TestImportGltf
    {
        ...
        {
            get { return Path.GetDirectoryName(Path.GetDirectoryName(Application.dataPath)); }
        }
        ...
        [Test]
        [MenuItem("Icosa/Dev/Test/Save as Separate")]
        public static void TestSaveAsSeparateWithMeshesInAsset()
        {
            ...
            {
                result = ImportGltf.Import(GltfSchemaVersion.GLTF1, reader, binLoader, IcosaImportOptions.Default());
            }
        ...
        [Test]
        [MenuItem("Icosa/Dev/Test/Save as Single")]
        ...
        private static GameObject DoImport(string gltfPath, IcosaImportOptions options, bool savePrefab = false)
        {
        ...
        [MenuItem("Icosa/Dev/Test/Import+save selected .gltf assets")]
        public static void TestImportSelection()
        {
            ...
            {
                DoImport(asset, IcosaImportOptions.Default(), true);
            }
        }

        [Test]
        [MenuItem("Icosa/Dev/Test/Import only/glTF1")]
        public static void TestImportGltf1()
        {
            DoImport(Path.Combine(RepoRoot, kAllBrush14), IcosaImportOptions.Default());
        }
        ...
        {
            DoImport(Path.Combine(RepoRoot, kComputer), IcosaImportOptions.Default());
        }

        [Test]
        [MenuItem("Icosa/Dev/Test/Import only/glTF2, scale x2")]
        public static void TestImportGltf2Scale()
        {
            IcosaImportOptions options = new IcosaImportOptions();
            options.rescalingMode = IcosaImportOptions.RescalingMode.CONVERT;
            options.scaleFactor = 2.0f;
            DoImport(Path.Combine(RepoRoot, kComputer), options);
        }

        [Test]
        [MenuItem("Icosa/Dev/Test/Import only/glTF2, target size=50")]
        public static void TestImportGltf2TargetSize()
        {
            IcosaImportOptions options = new IcosaImportOptions();
            options.rescalingMode = IcosaImportOptions.RescalingMode.FIT;
            options.desiredSize = 50;
            DoImport(Path.Combine(RepoRoot, kComputer), options);
        }

        [Test]
        [MenuItem("Icosa/Dev/Test/Import only/glTF2, target size=50, recenter")]
        ...
        [Test]
        [MenuItem("Icosa/Dev/Test/Import only/Goblets (GLTF2 transparent)")]
        public static void TestImportGltf2Transparent()
        {
            DoImport(Path.Combine(RepoRoot, kGoblets), IcosaImportOptions.Default());
        }

        [Test]
        [MenuItem("Icosa/Dev/Test/Import only/Shark (GLTF2 generated from FBX)")]
        public static void TestImportGltf2FromFbx()
        {
            DoImport(Path.Combine(RepoRoot, kShark), IcosaImportOptions.Default());
        }
        ...
        {
            Vector3 vn = IcosaClientInternal.MathUtils.CalculateNormal(v0, v1, v2);
