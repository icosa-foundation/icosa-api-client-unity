...
using IcosaClientInternal;  // NO_POLY_TOOLKIT_INTERNAL_CHECK
...
/// </summary>
public class IcosaApiClientCmd : MonoBehaviour {
  ...
  private List<IcosaAsset> currentResults = new List<IcosaAsset>();
  ...
  private void Start() {
    ...
    // Initialize Poly. We do this explicit initialization rather than have a IcosaToolkitManager on the
    ...
    IcosaApi.Init(new IcosaAuthConfig(BASE_URL, API_KEY, CLIENT_ID, CLIENT_SECRET));
  }
  ...
  private void AuthCallback(IcosaStatus status) {
    if (status.ok) {
      PrintLn("Signed in successfully as {0}.", IcosaApi.UserName);
      userProfileImage.sprite = IcosaApi.UserIcon;
    ...
    switch (args[0]) {
      case "signin":
        ...
        if (manualAccessToken != null) {
          ...
          IcosaApi.Authenticate(manualAccessToken, manualRefreshToken, AuthCallback);
        } else {
          PrintLn("Attempting {0} auth...", interactive ? "interactive" : "non-interactive");
          IcosaApi.Authenticate(interactive, AuthCallback);
        }
        break;
      case "signout":
        IcosaApi.SignOut();
      ...
      case "status":
        PrintLn("authenticated: {0}\nauthenticating: {1}\naccess token: {2}\nrefresh token: {3}",
            IcosaApi.IsAuthenticated, IcosaApi.IsAuthenticating, IcosaApi.AccessToken, IcosaApi.RefreshToken);
        break;
      default:
        PrintCommandHelp("auth");
        break;
  ...
  private void CmdList(string[] args) {
    IcosaListAssetsRequest req;

    if (args.Length > 0 && args[0] == "featured") {
      // Default list request (featured).
      req = IcosaListAssetsRequest.Featured();
    } else if (args.Length > 0 && args[0] == "latest") {
      // Default list request (latest).
      req = IcosaListAssetsRequest.Latest();
    } else {
      // Custom list request.
      req = new IcosaListAssetsRequest();
    }
    ...
    if (HasOpt(args, "-f")) {
      req.formatFilter = GetEnumOpt(args, "-f", IcosaFormatFilter.BLOCKS /* not used */);
    }
    ...
    IcosaApi.ListAssets(req, (IcosaStatusOr<IcosaListAssetsResult> res) => {
      if (!res.Ok) {
  ...
  private void CmdListMy(string[] args) {
    IcosaListUserAssetsRequest req;

    if (args.Length > 0 && args[0] == "newest") {
      // Default list request (newest).
      req = IcosaListUserAssetsRequest.MyNewest();
    } else {
      // Custom list request.
      req = new IcosaListUserAssetsRequest();
    }
    // Mutate the request according to parameters.
    ...
    IcosaApi.ListUserAssets(req, (IcosaStatusOr<IcosaListAssetsResult> res) => {
      if (!res.Ok) {
        PrintLn("Request ERROR: " + res.Status);
        return;
      }
  ...
  private void CmdShow(string[] args) {
    ...
    for (int i = 0; i < currentResults.Count; i++) {
      IcosaAsset asset = currentResults[i];
      PrintLn("[{0}]: {1}\n  ID: {2}\n  Author: {3}\n  Formats: {4}\n",
  ...
  private void ShowAssetDetails(int index) {
    ...
    IcosaAsset asset = currentResults[index];
  ...
  private void CmdImp(string[] args) {
    ...
    IcosaAsset assetToImport = currentResults[index];
    IcosaImportOptions options = new IcosaImportOptions();
    options.rescalingMode = GetEnumOpt(args, "-m", IcosaImportOptions.RescalingMode.FIT);
    ...
    IcosaApi.Import(assetToImport, options, (IcosaAsset asset, IcosaStatusOr<IcosaImportResult> result) => {
      if (!result.Ok) {
  ...
  private void CmdThumb(string[] args) {
    ...
    IcosaAsset assetToUse = currentResults[index];
    PrintLn("Fetching thumbnail... Please wait.");
    IcosaApi.FetchThumbnail(assetToUse, (IcosaAsset asset, IcosaStatus status) => {
      if (status.ok) {
  ...
  private static string FormatListToString(List<IcosaFormat> list) {
